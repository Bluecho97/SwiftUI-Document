...
// MARK: - get
private func getUserList(_ search: String = "all") async throws -> [APIModel] {
...
// MARK: - post
private func postUser(data: APIModel) async {
    // 1. URL 만들기
    let urlString = BaseURL.baseUrl.rawValue
    ...
    // 2. 새로운 데이터 생성
    let newUser = APIModel(id: nil, name: data.name, age: data.age, part: data.part)
...
// MARK: - PATCH
private func updateUser(id: Int, data: APIModel) async {
    // 1. URL 만들기
    let urlString = BaseURL.baseUrl.rawValue
    ...
    // 2. 업데이트할 데이터 구성
    let updatedUser = APIModel(id: nil, name: data.name, age: data.age, part: data.part)
